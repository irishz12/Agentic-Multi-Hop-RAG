...
export function CaseStudy({ successAnalysis, examplesReplay }: CaseStudyProps) {
  ...
  const exactHopStopMatches = examplesReplay.records.filter((record) => {
    const detail = detailById.get(record.qa_id);
    if (!detail) return false;
    return detail.num_agent_hops === record.hops.length && detail.stop_reason === record.stop_reason;
  }).length;
      ...
      <div className="space-y-16">
        ...
        <div>
          ...
          <p className="mb-10 max-w-[42rem] text-sm text-ink-muted">
            ...
            aggregate result. Hop count and stop reason reproduced exactly in {formatInt(exactHopStopMatches)} of{" "}
            {formatInt(examplesReplay.records.length)}; the exact final evidence-document set reproduced exactly in{" "}
            {formatInt(exactReplayMatches)} of{" "}
        ...
        <div>
          ...
          <ul className="mt-4 space-y-4">
            ...
            <li className="max-w-[42rem] text-[1.0625rem] leading-relaxed text-ink-muted">
              Generation is also not fully deterministic in its stopping decision: hop count and stop reason
              reproduced exactly in {formatInt(exactHopStopMatches)} of {formatInt(examplesReplay.records.length)}
              replayed questions. Retrieval itself showed minor variation on replay too: the exact final
              evidence-document set reproduced in {formatInt(exactReplayMatches)} of{" "}
              {formatInt(examplesReplay.records.length)} replayed questions — most likely because approximate
              nearest-neighbor search isn&rsquo;t guaranteed bit-identical across separate live queries, and because
              the controller&rsquo;s own stop/continue judgment is itself a hosted LLM call. Disclosed, not hidden.
            </li>
            <li className="max-w-[42rem] text-[1.0625rem] leading-relaxed text-ink-muted">
