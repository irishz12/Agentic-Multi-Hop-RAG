...
def test_always_agentic_projection_routes_everything_to_agentic():
    projection = always_agentic_projection(27, UnitCost(cost_usd=0.0012, latency_ms=8550.0))
...
def test_always_agentic_projection_requires_positive_n():
    with pytest.raises(ValueError):
        always_agentic_projection(0, UnitCost(0.0, 0.0))


def test_routed_workload_cheaper_than_always_agentic_when_most_queries_are_simple():
    unit_costs = {
        "hybrid_only": UnitCost(cost_usd=0.0, latency_ms=60.0),
        "hybrid_reranker": UnitCost(cost_usd=0.0, latency_ms=200.0),
        "agentic": UnitCost(cost_usd=0.0012, latency_ms=8550.0),
    ...
    always_agentic = always_agentic_projection(27, unit_costs["agentic"])
    assert routed.total_cost_usd < always_agentic.total_cost_usd
    assert routed.total_latency_ms < always_agentic.total_latency_ms
